    ...
    */
                    ...
                    this.validate = function(value) {
                        try {
                            ...
                            if ("string" === typeof value && isNaN(Number(value)) || _isArray(value)) new Assert().Length({
                                min: this.min,
                                max: this.max
                            }).validate(value); else new Assert().GreaterThanOrEqual(this.min).validate(value) && new Assert().LessThanOrEqual(this.max).validate(value);
                            return true;
                        } catch (violation) {
                ...
                Required: function() {
                    ...
                    this.validate = function(value) {
                        ...
                        try {
                            if ("string" === typeof value) new Assert().NotNull().validate(value) && new Assert().NotBlank().validate(value); else if (true === _isArray(value)) new Assert().Length({
                                min: 1
                            }).validate(value);
                        } catch (violation) {
                            throw new Violation(this, value);
  ...
  */
            ...
            var expect = {
                ...
                keys: function(obj) {
                    ...
                    var keys = [];
                    for (var i in obj) if (Object.prototype.hasOwnProperty.call(obj, i)) keys.push(i);
                    return keys;
                },
        ...
        ParsleyUI.prototype = {
            ...
            _insertErrorWrapper: function(fieldInstance) {
                var $errorsContainer;
                if ("string" === typeof fieldInstance.options.errorsContainer) if ($(fieldInstance.options.errorsContainer + "").length) return $(fieldInstance.options.errorsContainer).append(fieldInstance._ui.$errorsWrapper); else if (window.console && window.console.warn) window.console.warn("The errors container `" + fieldInstance.options.errorsContainer + "` does not exist in DOM");
                if ("function" === typeof fieldInstance.options.errorsContainer) $errorsContainer = fieldInstance.options.errorsContainer(fieldInstance);
            ...
            actualizeTriggers: function(fieldInstance) {
                ...
                }); else fieldInstance.$element.on(triggers.split(" ").join(".Parsley ") + ".Parsley", false, $.proxy("function" === typeof fieldInstance.eventValidate ? fieldInstance.eventValidate : this.eventValidate, fieldInstance));
            },
    ...
    */
            ...
            _bindConstraints: function() {
                ...
                for (var i = 0; i < this.constraints.length; i++) if (false === this.constraints[i].isDomConstraint) constraints.push(this.constraints[i]);
            ...
            _bindHtml5Constraints: function() {
                ...
                if ("number" === type) return this.addConstraint("type", "integer", undefined, true); else if (new RegExp(type, "i").test("email url range")) return this.addConstraint("type", type, undefined, true);
                return this;
            },
            // Internal only.
        ...
        Parsley.prototype = {
            ...
            handleMultiple: function(parsleyFormInstance) {
                ...
                if ("undefined" !== typeof name) $('input[name="' + name + '"]').each(function() {
                    if ($(this).is("input[type=radio], input[type=checkbox]")) $(this).attr(that.options.namespace + "multiple", multiple);
                });
                // Check here if we don't already have a related multiple instance saved
                if ($("[" + this.options.namespace + "multiple=" + multiple + "]").length) for (var i = 0; i < $("[" + this.options.namespace + "multiple=" + multiple + "]").length; i++) if ("undefined" !== typeof $($("[" + this.options.namespace + "multiple=" + multiple + "]").get(i)).data("Parsley")) {
